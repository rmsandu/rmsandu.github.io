...
- **Theme token source:** Add the Lovable Theme: Fresh Greens `:root` and `.dark` token blocks from `BRIEF.md` to `custom-styles.scss`. Existing local color variables should be replaced or mapped to `hsl(var(--primary))`, `hsl(var(--background))`, `hsl(var(--card))`, `hsl(var(--foreground))`, and related token references.
