...
def test_homepage_render_flow():
    ...
    assert "Vision AI Engineer" in html
    assert "Machine Learning Team Lead" in html
    assert "A brief timeline" not in html
    assert ">Path<" not in html
    assert (
        '<a href="https://www.cascination.com/en/ablasure">'
        "commercial product deployed in hospitals.</a>"
    ) in html
    assert (
        '<a href="https://ebird.org/profile/NTI4NTk0OQ/world">birdwatching</a>'
    ) in html
