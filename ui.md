...
- **Color:** Use Lovable Theme: Fresh Greens from `BRIEF.md` as the source of truth. Implementation should define the provided `:root` and `.dark` tokens in `custom-styles.scss` and consume them as `hsl(var(--primary))`, `hsl(var(--background))`, `hsl(var(--card))`, etc. Card surfaces should use `--card`; body/page surfaces should use `--background`; text should use the relevant foreground tokens.
